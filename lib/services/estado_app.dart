...
class EstadoApp extends ChangeNotifier {
  ...
  Future<void> _inicializar() async {
    try {
      ...
      if (authService.isAuthenticated) {
        print(
          '✅ [EstadoApp] Sesión de Supabase detectada, restaurando usuario autenticado...',
        );
    ...
    try {
      ...
      if (currentUser != null) {
        ...
        if (usuario != null) {
          ...
          print(
            '✅ [EstadoApp] Usuario autenticado restaurado: ${usuario.nombre}',
          );
        } else {
  ...
  Future<void> _restaurarSesionGuardada() async {
    try {
      ...
      print(
        '🔍 [EstadoApp] Verificando sesión guardada - esInvitado: $esInvitado',
      );
      ...
      if (esInvitado) {
        ...
        if (invitadoJson != null && invitadoJson.isNotEmpty) {
          try {
            ...
            print(
              '🔍 [EstadoApp] Campos del invitado - ID: $id, ID_Progreso: $idProgreso',
            );

            if (id == 0 || idProgreso == 0) {
              print(
                '⚠️ [EstadoApp] Invitado tiene IDs en 0 (modo offline), pero restaurando sesión',
              );
            }
            ...
            print(
              '✅ [EstadoApp] Usuario invitado restaurado: ${invitado.nombre} (ID: ${invitado.id})',
            );
          } catch (e) {
      ...
      if (progresoJson != null && progresoJson.isNotEmpty) {
        try {
          ...
          print(
            '✅ [EstadoApp] Progreso restaurado: nivel ${_progresoActual!['nivel']}',
          );
        } catch (e) {
  ...
  Future<void> _guardarEstado() async {
    try {
      ...
      await prefs.setBool(_keyEsInvitado, _esInvitado);
      if (_esInvitado && _usuarioInvitado != null) {
        ...
        print(
          '💾 [EstadoApp] JSON guardado en SharedPreferences: ${json.encode(invitadoJson)}',
        );
      } else {
        await prefs.remove(_keyUsuarioInvitado);
      }
      ...
      if (_progresoActual != null) {
        await prefs.setString(
          _keyProgresoActual,
          json.encode(_progresoActual!),
        );
        print(
          '💾 [EstadoApp] Guardando progreso: nivel ${_progresoActual!['nivel']}',
        );
      }
  ...
  Future<void> establecerUsuarioAutenticado(UsuarioModel usuario) async {
    ...
    try {
      ...
      final progreso = await progresoService.obtenerProgresoUsuario(
        usuario.idProgreso,
      );
      if (progreso != null) {
  ...
  Future<void> establecerUsuarioInvitado(InvitadoModel invitado) async {
    ...
    try {
      final progresoService = ProgresoService.instance;
      final progreso = await progresoService.obtenerProgresoInvitado(
        invitado.idProgreso,
      );
      if (progreso != null) {
        _progresoActual = progreso;
