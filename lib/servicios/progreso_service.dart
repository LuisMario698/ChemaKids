...
class ProgresoService {
  ...
  Future<List<ProgresoModel>> obtenerProgresoUsuarioJuego(int idUsuario, int idJuego) async {
    try {
      ...
      for (var progreso in progresos) {
        print('   🎯 Nivel ${progreso.nivel}: ${progreso.puntaje} pts (Racha: ${progreso.rachaMaxima})');
      }
      
      return progresos;
      
    } catch (e) {
      print('❌ Error al obtener progreso del usuario $idUsuario en juego $idJuego');
  ...
  Future<List<ProgresoModel>> obtenerProgresoUsuario(int idUsuario) async {
    try {
      ...
      for (var progreso in progresos) {
        ...
        if (progreso.rachaMaxima > juegoStats[progreso.idJuego]!['mejorRacha']) {
          juegoStats[progreso.idJuego]!['mejorRacha'] = progreso.rachaMaxima;
        }
      }
      
      juegoStats.forEach((juegoId, stats) {
        print('   🎮 Juego $juegoId: ${stats['niveles']} niveles, ${stats['puntajeTotal']} pts total, racha máx: ${stats['mejorRacha']}');
      });
  ...
  Future<List<ProgresoModel>> obtenerProgresoInvitadoJuego(int idInvitado, int idJuego) async {
    try {
      print('📊 Obteniendo progreso del invitado $idInvitado en juego $idJuego');
      ...
      print('❌ Error al obtener progreso del invitado $idInvitado en juego $idJuego');
      SupabaseService.instance.handleError('obtener progreso invitado-juego', e);
      return [];
    }
  }
  ...
  Future<ProgresoModel?> obtenerMejorPuntaje(int idUsuario, int idJuego) async {
    try {
      print('🏆 Buscando mejor puntaje del usuario $idUsuario en juego $idJuego');
      ...
      print('❌ Error al obtener mejor puntaje del usuario $idUsuario en juego $idJuego');
  ...
  Future<int> obtenerNivelMaximo(int idUsuario, int idJuego) async {
    try {
      print('🎯 Buscando nivel máximo del usuario $idUsuario en juego $idJuego');
      ...
      print('❌ Error al obtener nivel máximo del usuario $idUsuario en juego $idJuego');
  ...
  Future<Map<String, dynamic>> obtenerEstadisticasJuego(int idJuego) async {
    try {
      ...
      print('   ⭐ Puntaje promedio: ${estadisticas['puntajePromedio']?.toStringAsFixed(1)}');
      print('   🎯 Nivel promedio: ${estadisticas['nivelPromedio']?.toStringAsFixed(1)}');
